...
`monty` now has the historical 35-battle README roster, a root Swift package, a Monty core catalog, a first SwiftUI app shell, data packs for the three demo battles, and a core launch flow from side selection to shared-surface/debrief/persistence state. It does not yet have a live DZW-backed Monty battle session or a `MontyTest` app.
