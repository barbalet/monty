...
- **Cycles 41-60 complete:** Added `MontyOrderDiceLaunchState` to launch flow, generated staged order-cup dice from force groups, wired the shared order buttons through Monty and MontyTest, implemented explicit order assignment in `MontyDemoBoardSession`, exposed current/available orders plus retained Down/Ambush snapshot state, and added order-control accessibility IDs. Documentation lives in `docs/monty_order_dice_cycle_041_060.md`; the session still keeps legacy Move/Shoot/Assault/Phase fallbacks until cycles 61-80 replace phase advancement with a real draw/activation lifecycle.
- **Cycles 61-80 complete:** Added `MontyOrderDiceCycle80Catalog` sidebar details, direct human interaction previews, order-specific blocker explanations, and Monty AI activation decisions that reuse DZW's `HistoricalAutoplayOrderAdvisor`. The shared `HistoricalPlayableBattleView` now surfaces morale, pins, retained orders, target reactions, and vehicle damage details, while the Monty app AI control executes one order-dice activation instead of a full side phase. Documentation lives in `docs/monty_order_dice_cycle_061_080.md`; the remaining compatibility gap is the autoplay runner/debrief path, which still advances by phases until cycles 81-105 convert it to activation steps and turn-end cleanup.
